...
def unitary_from_angles(theta, phi, lambdaa, phase):
    ...
    # Rz
    rz_lambda = np.array(
        [[np.exp(-1j * lambdaa / 2), 0], [0, np.exp(1j * lambdaa / 2)]],
        dtype=np.complex128,
    )
...
def compare_zyz_decomposition_to_qiskit(unitary, show_details=False):
    ...
    qiskit_theta, qiskit_phi, qiskit_lambda, qiskit_phase = decomposer.angles_and_phase(
        unitary
    )
    ...
    qiskit_u = unitary_from_angles(
        qiskit_theta, qiskit_phi, qiskit_lambda, qiskit_phase
    )
...
def compare_controlled_unitary_to_qiskit(unitary, show_details=True):
    """Compare local Controlled Unitary with Qiskit's implementation."""

    controlled_qiskit_u = Operator(unitary).to_instruction().control(1)
    qc1 = QuantumCircuit(2)
    qc1.append(controlled_qiskit_u, [0, 1])
    controlled_qiskit_u_matrix = Operator(qc1).data

    theta, phi, lambdaa, phase = zyz_decomposition(unitary)
    qasm = controlled_unitary_circuit(unitary)
    qc2 = qasm3_loads(qasm)
    qc2_no_measure = qc2.remove_final_measurements(inplace=False)
    controlled_qasm_u_matrix = Operator(qc2_no_measure).data

    # print(controlled_qasm_u_matrix, controlled_qiskit_u_matrix)
    controlled_unitary_ok = np.allclose(
        controlled_qasm_u_matrix, controlled_qiskit_u_matrix, atol=1e-8
    )
    ...
    if show_details:
        ...
        print("Nossa controlled_unitary do qasm gerado:")
        print(controlled_qasm_u_matrix)
        print()

        print("Qiskit's controlled_unitary:")
        print(controlled_qiskit_u_matrix)
        print()

        print("Resultados:")
        print(
            f"  Comparação das implementações de controlled_unitary OK:   {controlled_unitary_ok}"
        )
        print("=" * 80)

    return controlled_unitary_ok
...
def test_zyz_decomposition_matches_qiskit():
    ...
    for i in range(100):
        ...
        result = compare_zyz_decomposition_to_qiskit(u, show_details=False)
    ...
    if failed_matrices:
        ...
        compare_zyz_decomposition_to_qiskit(failed_matrices[0][1], show_details=True)
        assert False, (
            f"Teste falhou em {len(failed_matrices)} matrizes. Primeira falha na matriz {failed_matrices[0][0] + 1}."
        )
    else:
